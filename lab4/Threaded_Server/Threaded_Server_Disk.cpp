...
void respond_to_request(task_container* container){
    ...
    if ( container->method.compare("POST")==0 ){
        ...
        std::string ending ("\n");
        final_return_message = value_string + ending + container->return_hash + ending;
    }
    ...
    while (running_flag) {
        ...
        if (request->parse() < 0) {
            ...
            if(request_method.compare("GET")==0){ //get
                ...
                char return_hash[50];
                int x = read_from_file(data_key, &(container->return_value), return_hash, &mutex1, &condition, &mutex2);
                container->return_hash = std::string(return_hash);
                ...
                write_to_file(data_key, container->value, md5(container->key).c_str(), &mutex1, &condition, &mutex2);
                respond_to_request(container);
                printf("Completed task_POST\n");
            }
            else if(request_method.compare("DELETE")==0){ //Delete
                DELETE_COUNT++;
                int x = delete_from_file(data_key, &mutex1, &condition, &mutex2);
                container->is_found = (x==0) ? true:false;
                respond_to_request(container);
